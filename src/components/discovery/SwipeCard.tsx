...
import { CheckCircle, DollarSign, Instagram, MapPin } from 'lucide-react';
    ...
    return (
        ...
        >
            <div className="relative w-full h-full rounded-3xl overflow-hidden bg-[#0A0A0A] shadow-2xl border border-white/5">
                ...
                {images.length > 1 && (
                    <div className="absolute top-4 left-0 right-0 flex gap-1 px-4 z-20">
                        {images.map((_, idx) => (
                            <div
                                key={idx}
                                className={`h-1 flex-1 rounded-full transition-all ${idx === currentImageIndex ? 'bg-white' : 'bg-white/30'
                                    }`}
                            />
                        ))}
                    </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 z-10 transition-all duration-300">
                    <div className="mb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {profile.lifestyle && profile.lifestyle.slice(0, 3).map((item, idx) => (
                                <span
                                    key={idx}
                ...
                {/* Swipe Indicators */}
                <motion.div
                    className="absolute top-1/3 left-8 z-30 pointer-events-none"
                    style={{
                ...
                <motion.div
                    className="absolute top-1/3 right-8 z-30 pointer-events-none"
                    style={{

...
export const ProfileDetailScreen: React.FC<Props> = ({ onNavigate }) => {
    return (
        <div className="max-w-md mx-auto relative min-h-screen bg-background-dark overflow-hidden">
            <nav className="absolute top-0 w-full z-20 flex justify-between items-center p-6 pt-12 bg-gradient-to-b from-black/60 to-transparent">
                <button onClick={() => onNavigate(Screen.Discovery)} className="w-10 h-10 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center text-white border border-white/10">
                    <Icon name="arrow_back" />
                </button>
                <button className="w-10 h-10 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center text-white border border-white/10">
                    <Icon name="more_horiz" />
                </button>
            </nav>

            <div className="relative h-[75vh] w-full">
                <img src="https://images.unsplash.com/photo-1542596594-649edbc13630?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover" />
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-background-dark border-t border-white/5 flex gap-4 z-50">
                ...
                <button onClick={() => onNavigate(Screen.Gifts)} className="flex-[2] h-14 rounded-xl bg-gradient-to-r from-primary to-[#b59a0b] text-background-dark font-bold text-lg flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(242,204,13,0.4)]">
